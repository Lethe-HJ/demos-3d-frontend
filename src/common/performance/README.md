...
- `channelGroup`: 行组名称，可选值：`'network'`, `'cache'`, `'worker'`, `'compute'`, `'render'`, `'backend'`
...
**方式一：使用 startRecord / endRecord（推荐用于异步操作）**

```typescript
// 开始记录
tracker.startRecord("event_id", "network", 0, "发送预处理请求");

// 执行操作
await fetch("/api/endpoint");

// 结束记录
tracker.endRecord("event_id");
```
...
async function loadData() {
  ...
  tracker.startRecord("preprocess", "network", 0, "发送预处理请求");
...
async function loadPerformance(sessionId: string) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (session) {
        session.records = [...session.records, ...backendRecords];
        const allTimes = session.records.flatMap((r) => [
          r.startTime,
          r.endTime,
        ]);
...
self.addEventListener("message", async (event) => {
  ...
  if (tracker) {
    tracker.endRecord("fetch_chunk");

    // 解析响应头中的性能数据
    const { recordPerformanceFromResponse } = await import(
      "../../common/performance/response-header"
    );
...
```typescript
// 方式一：startRecord / endRecord
tracker.startRecord("event_id", "compute", 0, "Worker 计算数据");
// ... 执行操作
tracker.endRecord("event_id");
...
self.addEventListener("message", async (event) => {
  ...
  try {
    ...
    if (tracker) {
      tracker.endRecord(`fetch_${chunkIndex}`);
...
##### `startRecord(eventId, channelGroup, channelIndex, msg)`
...
- `eventId`: 事件唯一标识符（必须与 `startRecord` 中的一致）
...
```typescript
// ✅ 好的命名
tracker.startRecord("preprocess_request", "network", 0, "发送预处理请求");
tracker.startRecord("chunk_0_fetch", "network", 0, "请求 Chunk 0");

// ❌ 不好的命名
tracker.startRecord("event1", "network", 0, "事件1");
```
...
self.addEventListener("message", async (event) => {
  ...
  const tracker = new PerformanceTracker({ enabled: true, sessionId });
